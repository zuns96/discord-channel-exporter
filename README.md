...
4. Select the export format:
   ...
   | **PDF** | Opens in new tab with "Save as PDF" button → print dialog |
...
3. Messages are formatted into the selected output format and downloaded via `chrome.downloads` API (PDF opens in a new tab with a print button for browser-native PDF saving).
...
- **다양한 내보내기 포맷** — HTML (Discord 다크 테마), CSV (Excel 호환), PDF (새 탭에서 인쇄 다이얼로그로 저장)
...
3. 메시지를 선택한 포맷으로 변환한 뒤 `chrome.downloads` API로 다운로드합니다 (PDF는 새 탭에서 브라우저 인쇄 기능으로 저장).
...
- PDF 내보내기는 새 탭에서 HTML을 열고 "PDF로 저장" 버튼을 통해 브라우저 인쇄 다이얼로그를 사용합니다.
